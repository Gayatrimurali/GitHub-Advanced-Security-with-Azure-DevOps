...
1. Click **Run** to run the pipeline configuration file which will set the dependency scanning
...
1. Switch to the **Gitchanges** tab and select **Configure** to add the username and email address which is required before committing changes.
...
1. Navigate to **Azure DevOps** click on **Repos** and select **Pull requests** and select **create a pull request** to push the commits from *codefix* to the *main*.
...
1. On the **New Pull request** page, add any work item from the list(which is mandatory) and click on **Create**
    ...
    > **Note:** The build will run automatically, initiating the dependency scanning task and publishing the results to Advanced Security, and the alert automatically close.
